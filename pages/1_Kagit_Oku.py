...
SABIT_API_KEY = "AIzaSyAn-VUAz2pgkLVzgw8XK8czrmJcJM8T1SE"

# Hafıza Ayarları (Resimler için)
if 'yuklenen_resimler_v3' not in st.session_state:
    st.session_state.yuklenen_resimler_v3 = []

# Hafıza Ayarları (Sınıf Verileri için - Main'den gelmezse diye önlem)
    ...
    if not SABIT_API_KEY or "API_ANAHTARINI" in SABIT_API_KEY:
        ...
        with st.spinner("Yapay zeka analiz yapıyor..."):
            try:
                ...
                - 4 madde istenip 4'ü yazıldıysa TAM PUAN ver.
                ...
                # --- 2. HAFIZAYA KAYDETME (ANALİZ İÇİN) ---
                # Hata veren kısım burasıydı, şimdi try bloğunun içinde ve güvenli
                ogrenci_verisi = {
                    "Ad Soyad": kimlik.get("ad_soyad", "Bilinmeyen"),
                    "Numara": kimlik.get("numara", "-"),
                    "Toplam Puan": toplam,
                }
                # Her soruyu tabloya ekle
                for soru in sorular:
